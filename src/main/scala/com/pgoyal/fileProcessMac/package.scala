...
package object fileProcess {
  ...
  var filePath = "/opt/cassandra/default/dataFiles/FileProcessing/"  
  var resourcesPath = "/Users/goyalpushkar/opt/cassandra/applicationstack/applications/wip/resources/"
  // "/opt/cassandra/applicationstack/applications/wip/resources/"
  ...
  def getEnvironmentName(environment: String): (String, String) = {
      ...
      val parsedFile = ConfigFactory.parseFile(new File( resourcesPath + "environment.conf"))
   ...
   def getCassandraConnectionProperties(environment: String): (String, String, String) = {
       ...
       logger.info("connectionHost - " + connectionHost + "\n" + "userName - " + userName + "\n" + "password - " + password )
  ...
  def poExistsSecondPass( poList: Map[String, String], secondPassList: WrappedArray[String] ): MMap[String, String] = {
     ...
     /*poList.foreach( x => logger.info("2nd Pass Value Found - " 
                                 + " Complete Value - " + x._2
  ...
  def poExistsFirstPass( poList: Map[String, String], firstPassList: WrappedArray[String] ): MMap[String, String] = {
     ...
     /*poList.foreach( x => logger.info("First Pass Value Found - " 
                                 + " Complete Value - " + x._2
                                 + " Vendor Name - " + x._2.toString().split(",").apply(1).dropRight(1)   //substring(1, poList.get(poKey).toString().split(",").apply(1).length()-1)
                                 + " Status - "  + x._2.toString().split(",").apply(0).drop(1)
                                ) )
